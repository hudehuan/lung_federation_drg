...
//                { field: 'mdcdm', title: '疾病编号', hidden: true },
                { field: 'mdcdm', title: '疾病编号', width: '80px' ,align: 'left', halign: 'center' },
                { field: 'mdcmc', title: '疾病名称', width: '350px' ,align: 'left', halign: 'center' },
        ...
        function reLoadRoleMdc(roleid){
            $('#r_datagrid').treegrid({
                method: 'get',
                url:'/SysRoleMdc/getMdcByRoleID?roleId='+ roleid+ "&rdm="+ parseInt(Math.random()*100000)
            });
        }
        function GetCheckBox(value, rec, index, Id) {
            var isCheck = rec[Id] == true ? "Checked" : "";
            var strHTML = "<input id='Chk_" + rec.mdcdm + "_" + Id + "' onclick='checkValue(this)' type='checkbox' " + isCheck + " />";
            return strHTML;
        }

        function checkValue(obj) {
            var CurSelectData = $('#r_datagrid').treegrid('find', obj.id.split("_")[1]);
            if (SaveMdcCode.indexOf(obj.id.split("_")[1]) == -1)
                SaveMdcCode.push(obj.id.split("_")[1]);
        ...
        function DoSavePower() {
            ...
            if (SaveMdcCode.length == 0) {
                layerInfo('未对数据做任何更改，无需保存!!!');
                return;
            }
            var updated = [];
            $.each(SaveMdcCode, function (i, n) {
            ...
            for(var i=0;i<updated.length;i++){
                var d = {
                    id:updated[i].id,
                    mdcdm:updated[i].mdcdm,
                    mdcmc:updated[i].mdcmc,
            ...
            effectRow = JSON.stringify(roleMdc);
            SaveMdcCode = [];
            roleMdc = [];
            $.ajax({
                type:"POST",
                url:"/SysRoleMdc/saveRoleMdc?rdm=" + parseInt(Math.random()*100000),
                dataType:"json",
                contentType:"application/json",
                data:effectRow,
                success:function(data){
                    reLoadRoleMdc(roleid);
                    layerMsg("保存成功！");
                },
                error:function(data){
                    layerMsg("保存出错！");
