...
<%--<div id="menu"  style="position:fixed;top: 10px;right: 20px;z-index:20000;font-size:12px;">
    ...
    <ul>
        ...
        <li><a href="#item3"><span></span>- -机构代码效率评分排名</a></li>
    </ul>
</div>--%>
...
<script>
    var jsonpData =
        {
            "title":{
                "column":"机构代码,所属区域,费用/rw,时间/rw,床日费用,CMI床日费用,,,效率评分,排序",
            },
            "rows":{
                "row":[
                    {
...
//        data = jsonpDataInit(data);  //jsonp数据格式化
    ...
    function setData1(data) {
        ...
        var datay=data.rows.row.slice(0,20);

        var x1=[],y1=[];
        for(var i=0;i<datay.length;i++){
            x1.push(datay[i].cell[0]);
            y1.push(datay[i].cell[1]);
        }
        for(var i=0;i<y1.length;i++){
            for(var j=i;j<y1.length;j++){
                if(Number(y1[i])<Number(y1[j])){
                    var str0 = y1[i];
                    y1[i] = y1[j];
                    y1[j] = str0;
                    var str1 = x1[i];
        ...
        var colors = ['#4ea397'];
        var option=dlchart.CreateLineChart("",x1,[{name:"",data:y1}],{grid: {
            left: '3%',
            right: '8%',
            bottom: '3%',
            containLabel: true
