...
<script>
    ...
    function setdata0(data){
        ...
        var option = dlchart.CreatePieChart('', [ ], [{
                name: "",
                data: [{name: "入组率", value: datay.cell[2], label: {
            normal: {
                formatter: '{d} %',
                    textStyle: {
                    fontSize: 28
    ...
    function setdata1(data){
        ...
        var option = dlchart.CreateBarChart("", chartdata[0].data, chartdata[2], {
            grid: {
                left: '3%',
                right: '4%',
                bottom: '3%',
    ...
    function setdata2(data){
        ...
        var option = dlchart.CreateBarChart("", chartdata[0].data, chartdata[1], {
            grid: {
                left: '3%',
                right: '4%',
                bottom: '3%',
    ...
    function loadTable(data,tableId) {
        ...
        $('#'+tableId).empty();
        if(data){
            var html = "<p>出&nbsp;&nbsp;&nbsp;&nbsp;院："+data.cell[1]+"人</p>"
                +"<a>未入组："+data.cell[3]+"人</a>";
            $('#'+tableId).html(html);
        }
    ...
    $('#daochu').click(function () {
        ...
        html2canvas(document.querySelector("#main")).then(function(canvas) {
           ...
           var triggerDownload = $("#daochuimg1").attr("href", canvas.toDataURL()).attr("download", n+'病案质量分析.png');
            triggerDownload[0].click();
           var tablehtml='<tr><td style="width: 1150px;height: 780px;"><img src="'+n+'病案质量分析.png"></td></tr>'
            tableToExcel('table',tablehtml,'病案质量分析分析');
        });
    });
</script>
</body>
