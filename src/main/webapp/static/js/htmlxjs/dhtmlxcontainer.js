	...
	this.obj.updateNestedObjects = function(fromInit) {
		...
		if (this.skin == "dhx_terrace") {
			...
			if (this.vs[this.av].grid) {
				...
				var gTop = (mtAttached||this._isWindow?5:0);
				var gBottom = (this._isWindow?5:0); // padding in window only
				var gLeft = (this._isWindow?5:0); // padding in window only
			...
			if (this.vs[this.av].layout) {
				...
				var gLeft = 14; // always
				*/
				var gTop = (this._isCell&&this._noHeader&&!mtAttached?0:5); // if layout's cell w/o hdr and w/o mt - 0, otherwise always
				var gBottom = (this._isCell&&this._noHeader?0:5)
				var gLeft = (this._isCell&&this._noHeader?0:5); 
	...
	*/
	...
	this.obj.adjust = function(adjustMT) {
		...
		if (this.skin == "dhx_terrace") {
			...
			var mtLRPad = 0;
			if (this._isWindow || this._isCell || this._isAcc || this._isTabbarCell) mtLRPad = 5;
			if (this._isCell && this._noHeader) mtLRPad = 0;
			
			// menu/toolbar top padding
			var mtTPad = 0;
			if (this._isWindow || this._isCell || this._isAcc || this._isTabbarCell) mtTPad = 5;
			...
			// layout's top
			var mBPad = ((adjustMT == true && !this.vs[this.av].toolbar) || this._isLayout ? 5 : 0);
			var tBPad = (adjustMT == true || this._isLayout ? 5 : 0);
			...
			if (this.vs[this.av].toolbar) {
				...
				if (mtTPad == 0 && this.vs[this.av].menu != null & this._isCell) mtTPad = 5; // if tabbar and menu attached and no header
